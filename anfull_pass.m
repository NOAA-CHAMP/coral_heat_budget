function [mcsst, LONS, LATS] = anfull_pass(dtstr, plotflgs, boundingbox)
%function [mcsst, LONS, LATS] = anfull_pass(dtstr, plotflgs, boundingbox)
%
% Map/plot AVHRR MCSST data from USF's FULL_PASS_HDF_SST dataset, for the
% year, jday and hour/min specified by string 'dtstr' (e.g., '2008320.2300').
% Optional second arg is a logical vector, as follows: 1:plot SST field.
% DEFAULT: [0]. Optional third arg: [lonW lonE latS latN] boundingbox for all
% plots. DEFAULT: boundingbox = [ -82.50 -79.10 +24.40 +25.90 ].
% 
% Returned values: plaid arrays (see 'meshgrid') of LONS, LATS, and
% multi-channel AVHRR sst (1km oC).
%
% Last Saved Time-stamp: <Wed 2009-03-11 10:25:46 Eastern Daylight Time gramer>


error('THIS PROCEDURE IS NOT READY FOR PRIME TIME!');
  ...
  if ( boundingbox(1) > boundingbox(1) || ...
...
function [fh, peakixes] = anmodis_plot_prop(hr,LONS,LATS,prp,prplim,prppeaks,ttl,ylbl,cmap)
...
%   ah(1) = subplot('position', [0.10 0.24 0.88 0.67]);
  ...
  set(fh, 'Name', ttlstr);
  figfname = sprintf('%s/%s.%s.png', figpath, hr, ylbl);
  print('-dpng', figfname);
