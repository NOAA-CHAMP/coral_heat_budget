function res = compare_flux_components(stn,per,RAPFX,KMPFX,ISPFX,TIDEPFX,WAVEPFX,substitute_field_names,commentstr)
%function res = compare_flux_components(stn,per,RAPFX,KMPFX,ISPFX,TIDEPFX,WAVEPFX,substitute_field_names,commentstr)
%
% Plot many annual or interannual heat-budget term climatologies from Gramer
% and Mariano (2012) and compare with implied heat flux from sea temperature.
%
% Last Saved Time-stamp: <Mon 2013-05-06 19:56:06 Eastern Daylight Time gramer>
  ...
  plot(res.sr.date,res.sr.data,'r-');
  plot(res.lr.date,res.lr.data,'b-');
  plot(res.asr.date,res.asr.data,'m-');
  plot(res.qbo.date,res.qbo.data,'k-.','LineWidth',1.5);
  plot(res.qlh.date,res.qlh.data,'r:','LineWidth',1.5);
  plot(res.qsh.date,res.qsh.data,'b:','LineWidth',1.5);
  plot(res.udT.date,res.udT.data,'k-','LineWidth',1.5,'Color',[.5,.5,.5]);
  plot(res.kd2T.date,res.kd2T.data,'k:','LineWidth',1.5,'Color',[.5,.5,.5]);
  plot(res.hc.date,res.hc.data,'k:','LineWidth',1.5,'Color',[.9,.3,.9]);
  if ( doClimTerms )
  ...
  plot(res.sr.date(mrkix),res.sr.data(mrkix),'rs');
  plot(res.lr.date(mrkix),res.lr.data(mrkix),'bs');
  plot(res.asr.date(mrkix),res.asr.data(mrkix),'m^');
  plot(res.qbo.date(mrkix),res.qbo.data(mrkix),'k+');
  plot(res.qlh.date(mrkix),res.qlh.data(mrkix),'ro');
  plot(res.qsh.date(mrkix),res.qsh.data(mrkix),'bo');
  ...
  plot(res.hc.date(mrkix),res.hc.data(mrkix),'k^','LineWidth',1.5,'Color',[.9,.3,.9]);
  if ( doClimTerms )
  ...
  plh(end+1)=plot(res.sr.date(1),res.sr.data(1),'rs-');
  legs(end+1) = {'Q_S_W'};
  plh(end+1)=plot(res.lr.date(1),res.lr.data(1),'bs-');
  legs(end+1) = {'Q_L_W'};
  plh(end+1)=plot(res.asr.date(1),res.asr.data(1),'m^-');
  legs(end+1) = {'\gammaQ_S_W'};
  plh(end+1)=plot(res.qbo.date(1),res.qbo.data(1),'k-.+');
  legs(end+1) = {'Q_b'};
  plh(end+1)=plot(res.qlh.date(1),res.qlh.data(1),'ro:');
  legs(end+1) = {'Q_L_H'};
  plh(end+1)=plot(res.qsh.date(1),res.qsh.data(1),'bo:');
  legs(end+1) = {'Q_S_H'};
  plh(end+1)=plot(res.udT.date(1),res.udT.data(1),'k-+','LineWidth',1.5,'Color',[.5,.5,.5]);
  legs(end+1) = {'F_qu\bullet\nablaT_s'};
  plh(end+1)=plot(res.kd2T.date(1),res.kd2T.data(1),'k:+','LineWidth',1.5,'Color',[.5,.5,.5]);
  legs(end+1) = {'K_H\nabla^2T_s'};
  plh(end+1)=plot(res.hc.date(1),res.hc.data(1),'k:^','LineWidth',1.5,'Color',[1,.2,1]);
  legs(end+1) = {'Hor. conv.'};
  if ( doClimTerms )
  ...
  set(legh,'FontSize',7);
  axis([min(res.t.date),max(res.t.date), -400,400]);
