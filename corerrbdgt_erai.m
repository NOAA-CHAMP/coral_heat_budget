function [dts,hs,dwth,hl,dwq] = corerrbdgt_erai(stn)
%function [dts,hs,dwth,hl,dwq] = corerrbdgt_erai(stn)
%
% Calculate latent and sensible heat flux errors using COARE 3.0a method
%
% Last Saved Time-stamp: <Wed 2011-08-24 16:51:12  Lew.Gramer>
...
qe=[0.18 0];%sea-air spechumid difference
  ...
  [six,aix,qaix,qsix,Wix,hix,qlhix,qshix,swix,lwix] = ...
      intersect_all_dates([], stn.ndbc_sea_t.date,stn.ndbc_air_t.date,...
                          stn.erai_spechumid.date,stn.ndbc_sea_spechumid.date,...
                          ...
                          stn.erai_dsrf.date,stn.erai_dlrf.date);
  ...
  qsa = stn.erai_spechumid.data(qaix);
  W = kts2mps(stn.ndbc_wind1_speed.data(Wix));
  h = stn.tmd_tide_i_depth.data(hix);
  qlh = real(stn.ndbc_erai_30a_latent_heat_flux.data(qlhix));
  qsh = real(stn.ndbc_erai_30a_sensible_heat_flux.data(qshix));
  dsrf = stn.erai_dsrf.data(swix);
  dlrf = stn.erai_dlrf.data(lwix);
