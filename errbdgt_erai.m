...
%
% Last Saved Time-stamp: <Sat 2011-01-29 11:41:50 Eastern Standard Time gramer>
...
clear corqerr         oldq            s2delTHC        uf2
...
stn = verify_variable(stn,'ndbc_erai_30a_net_heat_flux_24_hour_average');
...
[six,aix,dix,RHix,qaix,qsix,Uix,Vix,Ugix,hix,swhix,qswix,qlwix,qlhix,qshix,q0ix,h0ix,qeix] = ...
    intersect_all_dates([], stn.ndbc_sea_t.date,stn.ndbc_air_t.date,stn.erai_dew_t.date,...
                        stn.erai_relhumid.date,stn.erai_spechumid.date,stn.ndbc_sea_spechumid.date,...
                        stn.ndbc_wind1_u.date,stn.ndbc_wind1_v.date,...
                        stn.ndbc_erai_30a_cordiags.date,...
                        stn.tmd_tide_i_depth.date,...
                        stn.ww3_sigwavehgt.date, ...
                        stn.erai_dsrf.date,...
                        stn.erai_dlrf.date,...
                        stn.ndbc_erai_30a_latent_heat_flux.date,...
                        stn.ndbc_erai_30a_sensible_heat_flux.date,...
                        stn.ndbc_erai_30a_net_heat_flux.date,...
                        stn.ndbc_erai_30a_net_heat_flux_24_hour_average.date,...
                        stn.ndbc_erai_30a_ww3_fkeys_qe_dt.date);

dts = stn.ndbc_sea_t.date(six);

s = stn.ndbc_sea_t.data(six);					s2 = s.^2;
a = stn.ndbc_air_t.data(aix);					a2 = a.^2;
d = stn.erai_dew_t.data(dix);					d2 = d.^2;
RH = stn.erai_relhumid.data(RHix);				RH2 = RH.^2;
qs = stn.ndbc_sea_spechumid.data(qsix);				qs2 = qs.^2;
qa = stn.erai_spechumid.data(qaix);				qa2 = qa.^2;
U = stn.ndbc_wind1_u.data(Uix);					U2 = U.^2;
V = stn.ndbc_wind1_v.data(Vix);					V2 = V.^2;
Ug = real(stn.ndbc_erai_30a_cordiags.Ug(Ugix));			Ug2 = Ug.^2;
Cd = real(stn.ndbc_erai_30a_cordiags.Cd(Ugix));			Cd2 = Cd.^2;
Ch = real(stn.ndbc_erai_30a_cordiags.Ch(Ugix));			Ch2 = Ch.^2;
Ce = real(stn.ndbc_erai_30a_cordiags.Ce(Ugix));			Ce2 = Ce.^2;
h = stn.tmd_tide_i_depth.data(hix);				h2 = h.^2;
swh = stn.ww3_sigwavehgt.data(swhix);				swh2 = swh.^2;
qsw = stn.erai_dsrf.data(qswix);				qsw2 = qsw.^2;
qlw = stn.erai_dlrf.data(qlwix);				qlw2 = qlw.^2;
qlh = real(stn.ndbc_erai_30a_latent_heat_flux.data(qlhix));	qlh2 = qlh.^2;
qsh = real(stn.ndbc_erai_30a_sensible_heat_flux.data(qshix));	qsh2 = qsh.^2;

q0 = real(stn.ndbc_erai_30a_net_heat_flux.data(q0ix));
q0(~isfinite(q0)) = 0;
q02 = q0.^2;

qelt = real(stn.ndbc_erai_30a_ww3_fkeys_qe_dt.data(qeix));
...
h0 = ONEHOUR.*real(stn.ndbc_erai_30a_net_heat_flux_24_hour_average.data(h0ix)) ./ (rho.*Cp);
h02 = h0.^2;
...
if ( ~exist('cordts','var') )
  [cordts,corhs,cordwth,corhl,cordwq] = corerrbdgt_erai(stn);
end;
...
% Consistent with Markovic et al (2009), 14% relative error in ERAI Qsw
sQswi = 7 + (0.05.*qsw);					s2Qswi = sQswi.^2;
...
% Consistent with Markovic et al (2009), 3% relative error in ERAI Qlw
sQlwi = 3 + (0.01.*qlw);					s2Qlwi = sQlwi.^2;
...
s2h = 0.1 + (0.30.*h);						sh = sqrt(abs(s2h));
...
if ( doPlot )
  ...
  figure; maxigraph; hold on; plot(stn.ndbc_erai_30a_sensible_heat_flux.date,real(stn.ndbc_erai_30a_sensible_heat_flux.data),'k-'); plot(dts,(sign(qsh).*abs(sQsh)),'r.'); datetick3; titlename('\sigmaQ_S_H');

  [ix1,ix2] = intersect_dates(stn.ndbc_erai_30a_sensible_heat_flux.date,dts);
  [yr,mo,dy,hr,mn,sc] = datevec(stn.ndbc_erai_30a_sensible_heat_flux.date(ix1));
  dy = datenum(yr,mo,dy,0,0,0);
  shfld.date = unique(dy);
  shfld.data = grpstats(real(stn.ndbc_erai_30a_sensible_heat_flux.data(ix1)),dy);
...
if ( doPlot )
  ...
  figure; maxigraph; hold on; plot(stn.ndbc_erai_30a_latent_heat_flux.date,real(stn.ndbc_erai_30a_latent_heat_flux.data),'k-'); plot(dts,(sign(qlh).*abs(sQlh)),'r.'); datetick3; titlename('\sigmaQ_L_H');

  [ix1,ix2] = intersect_dates(stn.ndbc_erai_30a_latent_heat_flux.date,dts);
  [yr,mo,dy,hr,mn,sc] = datevec(stn.ndbc_erai_30a_latent_heat_flux.date(ix1));
  dy = datenum(yr,mo,dy,0,0,0);
  lhfld.date = unique(dy);
  lhfld.data = grpstats(real(stn.ndbc_erai_30a_latent_heat_flux.data(ix1)),dy);
...
% Time series of sea surface reflectances according to ERA Interim
Albedo = stn.erai_usrf.data(qswix) ./ qsw;
...
[asrfix,srfix] = intersect_dates(stn.absorbed_erai_srf.date,stn.erai_srf.date(qswix));
gamma = stn.absorbed_erai_srf.data(asrfix) ./ stn.erai_srf.data(qswix(srfix));
gamma(~isfinite(gamma)) = mean(gamma(isfinite(gamma)));
gamma(stn.absorbed_erai_srf.data(asrfix) < 0.1) = 0;
gamma2 = gamma.^2;
...
%%%% HACKOLA?!
%%%% DEBUG:
s2Qsw = ((1 - prctile(Albedo,99)).^2) .* s2Qswi;
s2Qsw(qsw<0.1) = 0;
sQsw = sqrt(abs(s2Qsw));
s2gammaQsw = (nanmedian(gamma(0<gamma&gamma<1))^2) .* s2Qsw;	sgammaQsw = sqrt(abs(s2gammaQsw));
...
if ( doPlot )
  ...
  figure; maxigraph; hold on; plot(stn.ndbc_erai_30a_net_heat_flux.date,real(stn.ndbc_erai_30a_net_heat_flux.data),'k-'); plot(dts,(sign(q0).*abs(sQ0)),'r.'); datetick3; titlename('\sigmaQ_0');

  [ix1,ix2] = intersect_dates(stn.ndbc_erai_30a_net_heat_flux.date,dts);
  [yr,mo,dy,hr,mn,sc] = datevec(stn.ndbc_erai_30a_net_heat_flux.date(ix1));
  dy = datenum(yr,mo,dy,0,0,0);
  q0fld.date = unique(dy);
  q0fld.data = grpstats(real(stn.ndbc_erai_30a_net_heat_flux.data(ix1)),dy);
