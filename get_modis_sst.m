function [LONS, LATS, sst] = get_modis_sst(dtstr,boundingbox)
%function [LONS, LATS, sst] = get_modis_sst(dtstr,boundingbox)
%
% Map/plot MODIS SST data for the Straits of Florida, for the year, jday and
% hour/minute specified by string DTSTR (e.g., '2008320.2300'). Optional
% second arg BOUNDINGBOX ([lonW lonE latS latN]) is applied to plot. DEFAULT:
% boundingbox = [ -83.00 -79.80 +24.00 +26.00 ]. BOUNDINGBOX may also be the
% 5-char code of an ICON station, e.g., 'SRVI2', Salt River.
% 
% Returned values: plaid arrays (see 'meshgrid') of LONS, LATS, sst (1km oC).
%
% Last Saved Time-stamp: <Mon 2010-10-18 11:35:33 Eastern Daylight Time gramer>
  ...
  % Find full filenames (incl. seconds of timestamp) for sst data

  %http://cyclops.marine.usf.edu/modis/level3/husf/florida/2010/166/1km/pass/final/MODIS.2010166.040036.florida.seadas_sst.hdf
  ...
  % Extract matching filename from USF's directory listing
  % RECENT:
  %  >MODIS.2009003.160747.florida.seadas_sst.hdf<
  patt = sprintf('>MODIS[.]%04d%03d[.]%02d%s[0-9][0-9]*[.]%s[.](seadas_sst)[.]hdf<', ...
  ...
  if ( isempty(begix) )
    warning('anmodis:TryingHistoricalData', ...
            ...
            baseurl, patt);
    patt = sprintf('>MODIS[.]%04d%03d[.]%02d%s[0-9][0-9]*[.]%s[.](sst)[.]hdf<', ...
                   jyear, jday, hr, mnstr, region);
    begix = regexp(fnames, patt);
