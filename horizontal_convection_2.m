function res = horizontal_convection_2(t,s,h,q0,bet,opts,dts,q0lp,wndlp)
%function res = horizontal_convection_2(t,s,h,q0,bet,opts,dts,q0lp,wndlp)
%
...
%
% Last Saved Time-stamp: <Thu 2013-04-18 17:02:01 Eastern Daylight Time gramer>
  ...
  % Periodicity of surface forcing (T or Tf in literature)
  st.s.date=st.q.date;
  ...
  lagoff = get_opt(opts,'convective_lag',3);
  ...
  coolt = get_opt(opts,'cooling_cutoff',0);
  warmt = get_opt(opts,'warming_cutoff',0);
  coolix = find(q0<coolt);
  zeroix = find(coolt<=q0 & q0<=warmt);
  warmix = find(q0>warmt);

  res.cooling_factor = get_opt(opts,'cooling_factor',1.0);
  res.warming_factor = get_opt(opts,'warming_factor',1.0);
  res.u(coolix) = res.cooling_factor*res.u(coolix);
  res.u(zeroix) = 0;
  res.u(warmix) = res.warming_factor*res.u(warmix);
  ...
  maxu = get_opt(opts,'maximum_velocity',2.00);
