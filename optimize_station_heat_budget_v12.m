...
%
% Last Saved Time-stamp: <Sun 2013-03-10 18:53:04 Eastern Daylight Time gramer>
  ...
  switch ( STNM ),
   ...
   case 'FWYF1',
    ...
    elseif ( use_old_options )
      ...
      doWarms = [true];
      kds = { ...
          [0.025,0.225,102] ...     % Best with hc_scaling=SU,warming_factor=0.66
            };
      advfacs = { ...
          [0.00,0.50,102] ...
                };
...
% %%%%%%%%%%???DEBUG
      ...
      doWarms = [true];
      kds = { ...
          [.038,.250, 67] ...     % Best with hc_scaling=SU,warming_factor=0.66
            };
      advfacs = { ...
          [0.00,1.0, 45] ...
                };
...
%%%%???DEBUG
  ...
  ocean_current_wind_multiplier = get_opt(stn.opts,'ocean_current_wind_multiplier',[]);
  if ( ~isempty(ocean_current_wind_multiplier) )
    stn.(sssfld) = ts_op(ts_fun(stn.(Wlpfld),@kts2mps),ocean_current_wind_multiplier,'*');
  ...
  if ( ~isfield(stn,hkmtxfld) )
    ...
    if( stn.opts.tidal_advection )
      ...
      stn.commentstr = [stn.commentstr,' (tidal adv.) '];
    else
      ...
      for cbdix=1:numel(cbds)
        ...
        for advix=1:numel(advfacs)
          ...
          for kthix=1:numel(kths)
            ...
            res = horizontal_convection(t,s,h,dT,bet,stn.opts,dts,dT,W);
            if ( isempty(strfind(stn.commentstr,' wf=')) )
              stn.commentstr = [stn.commentstr,' ',strrep(stn.opts.hc_scaling,'_','\_'),...
                                ' wf=',num2str(res.warming_factor)];
            end;
