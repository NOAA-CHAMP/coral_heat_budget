function res = pixel_heat_budget(pix_coords,reef_coords,RAPFX,KMPFX,ISPFX,TIDEPFX,WAVEPFX,substitute_field_names,opts)
%function res = pixel_heat_budget(pix_coords,reef_coords,RAPFX,KMPFX,ISPFX,TIDEPFX,WAVEPFX,substitute_field_names,opts)
%
% Calculate reef ocean heat budget for all sub-regions bounded by REEF_COORDS
% within pixel bounded by coordinates PIX_COORDS.  Most args are optional and
% specify fieldname prefixes: e.g., RAPFX=='erai' uses data fields from ECMWF
% Reanalysis-Interim (ERAI) for radiative fluxes and meteorological inputs;
% ISPFX=='ndbc' uses in situ sea/air temp. and winds. See the documentation
% of function STATION_HEAT_BUDGET for more details. 
%
...
%
% Specifying RES.(COMPARISONSFLD) plots a sea temperature that may be
% different from RES.(SFLD) for comparison with the heat budget results.
%
...
%
% Last Saved Time-stamp: <Sun 2014-09-28 18:38:50 Eastern Daylight Time gramer>


  % Save struct RES to a (*large*) MAT file before finishing?
  doSave = false;
  ...
  res.pixel_coords = pixel_coords;
  res.reef_coords = reef_coords;
  res.commentstr = '';
  ...
  res.TIDEPFX=TIDEPFX;
  res.WAVEPFX=WAVEPFX;
  res.substitute_field_names = substitute_field_names;
  res.QEPFX=QEPFX;
  res.DTPFX=DTPFX;
