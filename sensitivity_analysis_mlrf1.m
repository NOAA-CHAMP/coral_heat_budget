...
% disp('*************************************************');
% disp(['REDOING ',upper(stnm),' SENSITIVITY ANALYSIS: ',datestr(now)]);
% disp('*************************************************');
...
disp('******************** Attenuation Kd ********************');
%% Was:           [0.050,0.350, 91] ... %Best so far
%for kd = {[0.05,0.35,137],[0.05,0.30, 91],0.2};
% Is:          [.035,.250, 69] ...     % Best w/TIDAL ADVECTION, hc_scaling=SU,hc_warming_factor=0.66, consistent w/SMKF1
for kd = {[0.035,0.250,115],[0.035,0.200, 70],0.14};
...
fname=[stnm,'-sensitivity-','do-warm','-false'],
fld=regexprep(fname,'[.-]','_');
s.(fld)=calc_heat_budget_rmse(optimize_station_heat_budget(stnm,RAPFX,KMPFX,ISPFX,TIDEPFX,WAVEPFX,subs,struct('override_do_warm_adj',{false})),cf);
xlabel(strrep(fname,'_','\_')); if (doPrint); print('-dtiff',fullfile(figspath,[fname,'.tif'])); end;
...
fld=regexprep(fname,'[.-]','_');
s.(fld)=calc_heat_budget_rmse(optimize_station_heat_budget(stnm,RAPFX,'gom_hycom',ISPFX,TIDEPFX,WAVEPFX,subs),cf);
xlabel(strrep(fname,'_','\_')); if (doPrint); print('-dtiff',fullfile(figspath,[fname,'.tif'])); end;

fname=[stnm,'-sensitivity-','adv-dif','-fkeys_hycom'],
fld=regexprep(fname,'[.-]','_');
s.(fld)=calc_heat_budget_rmse(optimize_station_heat_budget(stnm,RAPFX,'fkeys_hycom',ISPFX,TIDEPFX,WAVEPFX,subs),cf);
xlabel(strrep(fname,'_','\_')); if (doPrint); print('-dtiff',fullfile(figspath,[fname,'.tif'])); end;

 % Use HYCOM sea temperature gradients, but empirical currents
fname=[stnm,'-sensitivity-','adv-dif','-gom_hycom-empirical'],
fld=regexprep(fname,'[.-]','_');
s.(fld)=calc_heat_budget_rmse(optimize_station_heat_budget(stnm,RAPFX,'gom_hycom',ISPFX,TIDEPFX,WAVEPFX,subs,struct('km_scale_advection',false)),cf);
xlabel(strrep(fname,'_','\_')); if (doPrint); print('-dtiff',fullfile(figspath,[fname,'.tif'])); end;
